...
export default function SearchBar({
    ...
    const customStyles = {
        control: (provided) => ({
            ...provided,
            fontSize: isFontLarge ? "24px" : "16px"
        }),
        option: (provided) => ({
            ...provided,
    ...
    return (
        <div className="bg-blue-500/20 dark:bg-neutral-900/60 pt-5 pb-7 sm:pb-8">
            <form
                className="search-form dark:bg-neutral-700 max-w-6xl 
                mx-auto bg-white border-2 border-white 
                rounded-md shadow-3xl px-4 pt-4 pb-0 
                flex flex-wrap gap-2 w-[90%]">
                {/* Pole wpisania nazwy aukcji */}
                <div className="w-full flex items-center">
                    <MdOutlineTitle className={`text-white text-3xl mr-2 ${isFontLarge ? "text-4xl" : "text-3xl"}`} />
                    <TextField
                        fullWidth
                        variant="outlined"
                        size={`${isFontLarge ? "medium" : "small"}`}
                        placeholder="Wpisz nazwę aukcji"
                        value={searchedTermInAuctionName}
                        onChange={handleSearchChange}
                        className='bg-white rounded text-5xl text-gray-700 flex-grow'
                        inputProps={{
                            style: { fontSize: isFontLarge ? "20px" : "16px" }
                        }} />
                </div>
                ...
                <div className="flex flex-1 min-w-0 items-center gap-2">
                    {/* Ikonka lokalizacji */}
                    <FaMapMarkerAlt className={`text-white text-3xl ${isFontLarge ? "text-4xl" : "text-3xl"}`} />
                ...
                <div className="flex flex-1 min-w-0 items-center gap-2">
                    {/* Ikonka kategorii */}
                    <FaStream className={`text-white text-3xl ${isFontLarge ? "text-4xl" : "text-3xl"}`} />
                ...
                {isOptionsExpanded && (<>
                    <div className="w-full pt-0">
                        <div className="flex flex-wrap gap-2 pb-4 items-center">
                            <div className="w-2/5 flex-1 flex items-center">
                                <GiPoland className={`text-white text-3xl mr-2 ${isFontLarge ? "text-4xl" : "text-3xl"}`} />
                                <Select
                            ...
                            <div className="flex-1 flex items-center">
                                <div
                                    ...
                                    {selectedSortOrder === 'ASC' ? (
                                        <BiSortUp
                                            className={`text-white text-3xl ${isFontLarge ? "text-4xl" : "text-3xl"} hover:text-blue-400 duration-100 ease-linear`} />
                                    ) : (
                                        <BiSortDown
                                            className={`text-white text-3xl ${isFontLarge ? "text-4xl" : "text-3xl"} hover:text-blue-400 duration-100 ease-linear`} />
                                    )}
                            ...
                            <div className="flex-1 flex items-center space-x-2">
                                {/* Ikonka ceny */}
                                <div>
                                    <TbMoneybag className={`text-3xl text-white ${isFontLarge ? "text-4xl" : "text-3xl"}`} />
                                </div>
                    ...
                    <div className="w-full flex justify-center">
                        <GiBroom className={`text-3xl text-white mr-2 ${isFontLarge ? "text-4xl" : "text-3xl"}`} />
                        <Button
                            variant="outlined"
