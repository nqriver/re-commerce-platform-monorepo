            ...
            <div className={`flex border-solid border-blue-500 dark:border-neutral-200 rounded-xl ${isDivFocused ? "border-2 shadow-2xl" : "border"}`}>
                <input
                    ...
                    {type === "password" && !isPasswordVisible && (
                        <AiFillEyeInvisible className="w-2/12 text-2xl/3 my-auto text-blue-500 dark:text-neutral-50 hover:cursor-pointer hover:text-blue-700 dark:hover:text-neutral-400 ease-linear duration-100" onClick={handleTogglePasswordVisibility}/>
                    )}
                    {type === "password" && isPasswordVisible && (
                        <AiFillEye className="w-2/12 text-2xl/3 my-auto text-blue-500 dark:text-neutral-50 hover:cursor-pointer hover:text-blue-700 dark:hover:text-neutral-400 ease-linear duration-100" onClick={handleTogglePasswordVisibility}/>
                    )}
            </div>
        </div>
    )
